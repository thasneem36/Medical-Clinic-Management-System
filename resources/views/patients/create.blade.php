...
<div class="max-w-[1440px] mx-auto">
    ...
    <div class="mb-stack-lg flex justify-between items-end">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-on-surface mb-2">Register New Patient</h2>
            <p class="font-body-md text-body-md text-on-surface-variant">Enter the patient's details below to create a new record.</p>
        </div>
    </div>

    <!-- Registration Form Card -->
    <div class="bg-[#f8fafc] border border-[#e2e8f0] rounded-xl shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.05)] p-8">
        <form action="{{ route('patients.store') }}" method="POST" class="space-y-stack-lg">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
                    <div class="flex flex-col gap-2">
                        ...
                        <input class="min-h-[44px] px-4 rounded-lg border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-body-md font-body-md @error('first_name') border-error @enderror" id="firstName" name="first_name" type="text" value="{{ old('first_name') }}" required>
                        @error('first_name')
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input class="min-h-[44px] px-4 rounded-lg border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-body-md font-body-md @error('last_name') border-error @enderror" id="lastName" name="last_name" type="text" value="{{ old('last_name') }}" required>
                        @error('last_name')
                            <p class="text-error text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <input class="min-h-[44px] px-4 rounded-lg border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-body-md font-body-md @error('age') border-error @enderror" id="age" name="age" type="number" value="{{ old('age') }}" required>
                        @error('age')
                            <p class="text-error text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <input class="min-h-[44px] px-4 rounded-lg border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-body-md font-body-md @error('email') border-error @enderror" id="email" name="email" type="email" value="{{ old('email') }}">
                        @error('email')
                            <p class="text-error text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex flex-col gap-2 md:col-span-2">
                        ...
                        <input class="min-h-[44px] px-4 rounded-lg border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-body-md font-body-md @error('address') border-error @enderror" id="address" name="address" type="text" value="{{ old('address') }}">
                        @error('address')
                            <p class="text-error text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-stack-md mt-stack-lg border-t border-outline-variant">
                <a href="{{ route('patients.index') }}" class="px-6 py-2.5 rounded-lg border-2 border-secondary text-secondary font-label-md text-label-md hover:bg-surface-variant transition-colors min-h-[44px]">
                    Cancel
                </a>
