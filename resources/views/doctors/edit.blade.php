...
<div class="max-w-[1440px] mx-auto">
    ...
    <div class="mb-stack-lg flex justify-between items-end">
        <div>
            <h2 class="font-headline-lg text-headline-lg text-on-surface mb-2">Edit Doctor</h2>
            <p class="font-body-md text-body-md text-on-surface-variant">Update doctor information below.</p>
        </div>
    </div>

    <!-- Edit Form Card -->
    <div class="bg-[#f8fafc] border border-[#e2e8f0] rounded-xl shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.05)] p-8">
        <form action="{{ route('doctors.update', $doctor->id) }}" method="POST" class="space-y-stack-lg">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
                    <div class="flex flex-col gap-2">
                        ...
                        <input class="min-h-[44px] px-4 rounded-lg border border-outline-variant bg-surface focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all text-body-md font-body-md @error('name') border-error @enderror" id="name" name="name" type="text" value="{{ old('name', $doctor->name) }}" required>
                        @error('name')
            ...
            <div class="flex items-center justify-end gap-4 pt-stack-md mt-stack-lg border-t border-outline-variant">
                <a href="{{ route('doctors.show', $doctor->id) }}" class="px-6 py-2.5 rounded-lg border-2 border-secondary text-secondary font-label-md text-label-md hover:bg-surface-variant transition-colors min-h-[44px]">
                    Cancel
                </a>
